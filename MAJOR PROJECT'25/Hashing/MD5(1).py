...
def generate_hash(file_path, algo="md5"):
    """Generate hash (MD5) of the given PDF file"""
...
def encrypt_pdf(input_pdf, output_pdf, hash_output):
    ...
    hash_value = generate_hash(input_pdf, "md5")
...
def verify_integrity(original_hash, received_pdf, algo="md5"):
...
def verify_pdf():
    ...
    if file_path and hash_file:
        ...
        is_valid = verify_integrity(original_hash, file_path, "md5")
        if is_valid:
...
# === Stylish Heading ===
header = ttk.Label(
    root, text="MD-5", font=("Arial", 18, "bold"), bootstyle="info"
)
header.pack(pady=10)

tk.Label(root, text="\U0001F512 PDF Security System", font=("Arial", 16, "bold"), bg="#f0f0f0").pack(pady=15)

frame = tk.Frame(root, bg="#f0f0f0")
frame.pack(pady=10)

tk.Button(frame, text="\U0001F510 Encrypt & Generate Hash", font=("Arial", 12), bg="#4CAF50", fg="white",
          command=select_pdf_encrypt, width=25).pack(pady=5)

tk.Button(frame, text="\U0001F513 Decrypt PDF", font=("Arial", 12), bg="#008CBA", fg="white", command=select_pdf_decrypt,
